  ...
  <div class="wrapper">
    ...
    <div style="display: flex; ">
      <div class="cctv-main" v-for="item in cctvList" v-bind:key="item.cctvId">
        ...
        <canvas :id=item.posName style="width: 320px; height: 240px; margin:10px;">
        </canvas>
        <button @click="fullScreenModal=true; canvasId=item.posName+'modal'; websocketUrl=item.websocketURL; disconnectState = true;[disconnect()]">click</button>
      </div>     
    </div>
    ...
    getWebsocketVideo: function(){
      ...
      if(this.disconnectState === false){
        ...
        for(let i=0;i<this.cctvListLength;i++){
          ...
          this.canvasView[i] = document.getElementById(this.cctvList[i].posName);   //'test-cctv'+String(this.cctvList[i].cctvId
